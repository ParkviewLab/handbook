...
Edit the template, open a PR into `develop`, release; the change reaches every machine with the next `git pull` of `main`. A new agent needs a row in the table above and, if it writes, a statement of what it writes. A read-only definition carries the two wiki read tools and the wiki section that explains them; that section is one text held in every read-only definition, so it changes in all of them in one pull request, and a new read-only definition carries it too. A new skill is a directory `templates/skills/<name>/` holding a `SKILL.md`, which the same installer links into `~/.claude/skills/`; where a skill is the operational copy of a page here, the two change in the same pull request. An addition worth designing before it is built is proposed in the org's wiki, on its Proposals shelf, until it is built; then it retires into this page and [`agents-why.md`](agents-why.md), and whatever stays genuinely open remains on the shelf as its own small proposal. The model and effort of an agent change only for a reason recorded in [`agents-why.md`](agents-why.md).
