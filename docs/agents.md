...
- **Reviewers are dispatched together.** A docs PR gets `docs-reviewer`, `northstar-reviewer`, `docs-currency-checker`, and, where a changed document has a twin, `html-drift-checker`, in one turn, in parallel; a release gets `release-preflight` and `docs-currency-checker` together. That is where the speed comes from.
