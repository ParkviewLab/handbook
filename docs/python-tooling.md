...
The ParkviewLab Python stack is **uv + ruff + ty + hatchling**, on **Python 3.13**, with a `src/` layout. Every Python repo's `pyproject.toml` is nearly identical — start from the template ([`pyproject.toml.template`](../templates/pyproject.toml.template)) and change only the name/description/deps.

## The stack

- **[uv](https://docs.astral.sh/uv/)** — package manager and runner. `uv.lock` is **committed** (reproducible builds; see [`ci.md`](ci.md)). `uv sync` installs; `uv run <cmd>` runs in the project env. Each worktree has its own env (run `uv sync` per worktree).
...
- **ruff:** `line-length = 110`, `target-version = "py313"`, `select = ["E","F","W","I","B","UP","SIM","C4","RUF"]`, `ignore = ["E501"]` (the formatter handles wrapping), and `tests/**` ignores `B011` (`assert False` in tests).
- **ty:** `[tool.ty.src] include = ["src"]`, `[tool.ty.environment] python-version = "3.13"` (this exact shape in deco-assaying/smalt-mcp/ebony-enriching; a few repos vary or omit `[tool.ty]` — bring new repos to this shape).
