...
- **Whatever `main` holds is published, stale or not.** A pre-release sentence ("no image has been published yet"), an open question that was answered, or a work plan left in a design document ships as the release's documentation. The release preflight therefore includes `docs-currency-checker` ([`agents.md`](agents.md)) over `README.md` and `docs/`, and a stale statement is fixed on `develop` before the promotion rather than corrected by a second release ([`documentation.md`](documentation.md#documents-and-records)).
