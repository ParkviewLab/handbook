...
- The job summary gives the dev-tools version the release ran, the range, every listed pull request with the route by which it was recognised, the pull requests left out as carrying other work or as already shipped, the bookkeeping commits with their reasons, and every warning, among them a release that raises the major version and lists no breaking change. Warnings never enter the notes and never fail the job.
