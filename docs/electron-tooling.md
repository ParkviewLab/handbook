...
- Changelog automation is the same as everywhere else: the `changelog` job runs dev-tools' shared `generate-changelog` at a pinned release, and nothing is copied into the repo (see [`commits-and-changelogs.md`](commits-and-changelogs.md)).
