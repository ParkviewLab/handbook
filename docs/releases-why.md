...
Two consequences of going first are recorded here because they are corrections, not form. NR08: the reason `release-txt.yml` gave for having no changelog job, that a docs repository "has no Conventional-Commit signal to categorise", was false of dev-tools, whose merged pull requests carry typed titles and which ships the changelog generator itself. The true reason, now in the documents part and in both pages, is that a documents repository keeps no `CHANGELOG.md` to generate and commit back to `main`, and GitHub's generated notes are its release record: they already list every pull request merged since the previous tag, each with its link. And the real-history set of the changelog generator's acceptance tests, which had been named by language, was relabelled by version file and publish target, since the script reads the version file and its lock file: `pyproject.toml` with GHCR (paper-boxing), `pyproject.toml` with PyPI (cogrind-workshop), `pyproject.toml` with PyPI and GHCR (smalt-mcp), `package.json` with npm and GHCR (jonobones), `package.json` with installers (pensa-grex), and, pending a first repository, `Cargo.toml` with installers (pensa-forma once it has releases) and `npm only`. "Node, GHCR only" left the set, its two dimensions being covered by paper-boxing and jonobones. That relabelling was made in its own dev-tools pull request.

### One dev gate for every target (T9), and the installers' dev build (T10)

T9 was ruled for the real-merges build, whose decision 5 it settles, and not for the parts this change ships: one dev gate for every target, the installers part writing the computed dev version into the repository's version file in its workspace as the registry parts will, nothing committed, and `git dev-release` keyed on its declared input alone. The alternative, installers keeping a committed dev marker whilst the registry targets computed theirs, would have made two dev gates out of one. Until that build lands, no dev part writes a version: the dev gate reads the version file as checked out, and `git dev-release` sets the dev version, commits it and pushes it to `develop`.
