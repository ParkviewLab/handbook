...
9. Pins of `dev-tools` scripts: every workflow checkout of `ParkviewLab/dev-tools` on `develop`, the documentation site's excepted, names the newest dev-tools release whose own release run passed, that is, the newest `vX.Y.Z` whose `release.yml` run concluded `success` (`gh run list -R ParkviewLab/dev-tools --workflow release.yml --json headBranch,conclusion`), by the full SHA of its commit (`gh api repos/ParkviewLab/dev-tools/commits/<tag> --jq .sha`). A pin that lags behind it is a blocker: under the pin policy (`docs/ci.md`, "Shared dev scripts: `dev-tools`"), a pull request moves it before the promotion, since this release's notes would otherwise be written by an older script and stay as published.
