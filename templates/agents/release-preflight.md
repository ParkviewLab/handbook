...
9. Pins of `dev-tools` scripts: every workflow checkout of `ParkviewLab/dev-tools` on `develop`, the documentation site's excepted, names a dev-tools release at or after its floor, by the full SHA of the tag's commit (`gh api repos/ParkviewLab/dev-tools/commits/<tag> --jq .sha`). The floor is the newest `vX.Y.Z` whose `release.yml` run concluded `success` (`gh run list -R ParkviewLab/dev-tools --workflow release.yml --json headBranch,conclusion`) and whose range from the previous tag touched the pinned script's file (`gh api repos/ParkviewLab/dev-tools/compare/<previous tag>...<tag> --jq '.files[].filename'`). A pin below its floor is a blocker: under the pin policy (`docs/ci.md`, "Shared dev scripts: `dev-tools`"), a pull request moves it to the newest release whose run passed before the promotion, since this release's notes would otherwise be written by a script that a later release has changed, and would stay as published. A pin at or after its floor is current, however many releases that left its script unchanged have come since.
10. Documents: this preflight does not verify that `README.md` and `docs/` are current; `docs-currency-checker` does, and the session dispatches it alongside you (`docs/agents.md`). Say in the report that its verdict is a separate input to the release decision.
