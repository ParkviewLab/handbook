...
- Changelog and CI (`docs/ci.md`, `docs/commits-and-changelogs.md`): `cliff.toml` verbatim from the template where the repo's release has a `changelog` job; `reuse.yml` and `version-guard.yml` present, the test workflow of the repo's stack, `license-check.yml` where the repo's license requires it; actions pinned exactly; GHCR tags include `latest` where an image is built.
